...
mouse_to_human_ortholog_gene_expression_mapping <- function(df.expr, type_mouse_gene_ids="mgi") {
  ...
  if (type_mouse_gene_ids=="mgi") {
    ...
    rw.lower <- tolower(rownames(df.expr))
    
    ### Genes not found
    bool.not_found <- !((rw.lower) %in% df.gene_name_mapping$gene_name_optimal) 
    not_mapped_from_MGI_to_ensembl.n_genes <- sum(bool.not_found) # n genes could not be mapped
    not_mapped_from_MGI_to_ensembl.genes <- rw.lower[bool.not_found] # e.g Fam150a not found, but this is because it is called ALKAL1. Fam150a is listed as a synonym in Emsembl now.
    
    ### Match
    idx.match <- match(rw.lower, df.gene_name_mapping$gene_name_optimal) # find mapping idx. 'nomatch' is set to NA
    ...
    bool_dups <- duplicated(genes.ensembl, incomparables=NA) #changed
    genes.ensembl[bool_dups] <- NA
...
add_ensembl_ids_from_entrez <- function(df, colname_geneids_from="entrez", colname_geneids_to="ensembl_gene_id") {
  ...
  file.mapping <- "/raid5projects/timshel/sc-genetics/sc-genetics/data/gene_annotations/gene_id_mapping.hsapiens.ensembl_entrez.txt.gz"
